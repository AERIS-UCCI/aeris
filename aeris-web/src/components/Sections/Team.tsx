...
const people: Person[] = [
  {
    ...
    role: "Tec. Medica (Líder, Diseñadora)",
    imageUrl:
  ...
  {
    name: "Juddy Peña",
    role: "Tecnología Médica (Speaker)",
    imageUrl:
  ...
  return (
    <div className="bg-white py-32">
      <div className="mx-auto max-w-7xl px-6 text-center lg:px-8">
        ...
        <ul
          role="list"
          className="mx-auto mt-20 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 sm:grid-cols-2 lg:max-w-none lg:grid-cols-3"
        >
          {people.map((person) => (
            <li key={person.name}>
              <img
                src={person.imageUrl}
                alt={person.name}
                className="mx-auto size-56 rounded-full outline-1 -outline-offset-1 outline-black/5 object-cover"
              />
